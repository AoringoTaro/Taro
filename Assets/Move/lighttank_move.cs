...
public class lighttank_move : MonoBehaviour {
	...
	void Start () {
		moverule = GameObject.Find ("moverule_lighttank");
	...
	void Update () {
		...
		if (turn == 0) {
			...
			if (clickedGameObject == this.gameObject) {
				if (Input.GetMouseButtonDown(0)) {
					if (clickedGameObject.tag == "Redteam/lighttank") {
						if(movesign == 1){
							if (clickedGameObject == this.gameObject) {
								before = hit.point;
		...
		if (turn == 1) {
			...
			if (clickedGameObject == this.gameObject) {
				if (Input.GetMouseButtonDown(0)) {
					if (clickedGameObject.tag == "Blueteam/lighttank") {
						if(movesign == 0){
							if (clickedGameObject == this.gameObject) {
								before = hit.point;
								moverule.SetActive (true);
