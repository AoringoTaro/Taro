...
public class carryer_move : MonoBehaviour {
	...
	GameObject cameras;
	//Quaternion rotation = Quaternion.Euler(-f,0f,0f);
	//Vector3 mainbefore = new Vector3(0f,0f,0f);
	//Vector3 mainnow = new Vector3(0f,0f,0f);

	// Use this for initialization
	void Start () {
		moverule = GameObject.Find ("moverule_carryer");
	...
	void Update () {
		...
		if (turn == 0) {
			...
			if (clickedGameObject == this.gameObject) {
				if (Input.GetMouseButtonDown(0)) {
					if (clickedGameObject.tag == "Redteam/carryer") {
						if(movesign == 1){
							if (clickedGameObject == this.gameObject) {
								before = hit.point;
		...
		if (turn == 1) {
			...
			if (clickedGameObject == this.gameObject) {
				if (Input.GetMouseButtonDown(0)) {
					if (clickedGameObject.tag == "Blueteam/carryer") {
						if(movesign == 0){
							if (clickedGameObject == this.gameObject) {
								before = hit.point;
								moverule.SetActive (true);
